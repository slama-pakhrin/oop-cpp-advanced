...
namespace sdds
{
	...
	struct Profile
	{
		...
		{
			if (m_address.number < 0 || !isValidPostalCode(m_address.postal_code))
			{
		...
		{
			if (postalCode.size() != 7)
			{
				return false;
			}
			if (!isalpha(postalCode[0]) || !isdigit(postalCode[1]) || !isalpha(postalCode[2]) ||
				!isdigit(postalCode[4]) || !isalpha(postalCode[5]) || !isdigit(postalCode[6]))
			{
				return false;
			}
			if (postalCode[3] != ' ' || !isupper(postalCode[0]) || !isupper(postalCode[2]) || !isupper(postalCode[5]))
			{
				return false;
			}
			return true;
